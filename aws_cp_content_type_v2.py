...
def count_total_pages(paginator, bucket_name, prefix):
    total_pages = 0
    for _ in paginator.paginate(Bucket=bucket_name, Prefix=prefix, PaginationConfig={'PageSize': 1000}):
        total_pages += 1
    return total_pages
...
def update_content_type(bucket_name, prefix):
    ...
    try:
        ...
        for idx, page in enumerate(page_iterator, start=1):
            ...
            if 'Contents' in page:
                # Lista para armazenar objetos a serem processados nesta página
                objects_to_process = []
                ...
                log_message = (f"Página {idx}/{total_pages} processada. "
                               ...
                               f"Arquivos com 'application/octet-stream': {total_octet_stream_files}, "
                               f"Arquivos atualizados: {total_updated_files}")
                print(log_message)
